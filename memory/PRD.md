...
## Engineering-Grade Validation (iteration 7 — Sep 7, 2026)
- Validation steps now carry **tolerance + spec reference + measurement** and are logged with a real audit trail (user ask: "wall plumb ± 1/4 in, vertical rebar 16 in O.C. — measured value, tolerance, plan/spec reference, photo, and who verified it").
- **Backend** (`server.py`): `ValidationStep` gains `requires_measurement`, `unit`, `tolerance`, `spec_reference` (+ same on Create/Patch). `ValidationCheck` (per-entry record) gains `measured_value`, `verified_by`, `tolerance`, `spec_reference` (snapshot for permanent audit). Create/patch pass-through verified via curl.
- **Admin → Validation Rules** (`TasksAdmin.jsx`): new `RuleEditor` lets managers define Description + Target/Tolerance + Plan/Spec Reference + "Requires measured value" (with unit) + "Requires photo". Existing approved rules are inline-editable (pencil). Rules display Tol / Spec / Measure / Photo badges.
- **Field logging** (`FieldView.jsx` TaskSheet): each step shows its tolerance + spec chips; when `requires_measurement` a **Measured Value** input (with unit + target placeholder) appears; a **Verified By** field (defaults to logged-in crew member, editable, choice 2a) appears once pass/fail is set. Measured value is free-text (choice 1a); tolerance is guidance, crew marks pass/fail manually (choice 3b). All fields persisted into the entry's validation records.
- **Tested**: backend create-rule + log-entry round-trip via curl (tolerance/spec/measured/verified all persist); UI verified via screenshots — admin rule editor badges + field measured/verified inputs render correctly. Offline queue carries the new fields unchanged.

## Nathan2/Hermes MCP Server (iteration 9 — Sep 7, 2026)
Applied user-supplied patch `nathan2-something-better-mcp.patch` — adds a self-contained `mcp_server/` (stdio MCP server) so an external agent (Nathan2/Hermes) can operate PLUMBLINE via the API. Additive only; does not touch backend/ or frontend/ or app behavior.
- `mcp_server/server.py`: 13 tools over `mcp` 2.2.0 — read (health, list_jobs, job_overview, list_tasks, task_details, crew_stats, job_activity, common_mistakes) + safe writes (create_job, create_task, update_job, update_task, log_progress). No delete/reset/bulk-delete/settings/import/AI-rule tools. Writes gated by `SOMETHING_BETTER_WRITES_ENABLED`; failed validations require fix_notes; empty entries rejected.
- Config via env: `SOMETHING_BETTER_API_URL` (points at deployed `/api`), optional `SOMETHING_BETTER_API_TOKEN`. README documents Hermes host install + `~/.hermes/config.yaml` block. Runs on Nathan2's host, NOT as a public endpoint (app still has no auth).
- **Verified**: all 13 tools map to existing backend endpoints; bundled pytest suite 6/6 pass in isolated venv; live read smoke test against preview API (health, list_jobs pagination, job_overview dashboard) all 200 OK and render correctly.

User asks: auto-flag measured values beyond tolerance & warn the foreman; add a company Live Feed with a BYOK AI bot (`@titanicf_bot`) that can chat and make changes in the app. User choices: bot handle `@titanicf_bot`; provider = bring-your-own key (choice 2B); full edit access (choice 3F); one company-wide feed with system events (4A + yes); auto-fail on out-of-tolerance (5A).
- **Out-of-tolerance engine** (`backend/tolerance.py` + `frontend/src/lib/tolerance.js`, mirrored parsers, zero-AI): parses tolerances like "Plumb ± 1/4 in", "16 in O.C. ± 1/2 in", "min 30 in", "no more than 1/8 in" and fraction/mixed/decimal measured values.
  - Field TaskSheet gives instant feedback: out-of-tolerance turns the input red, auto-selects FAIL, shows "OUT OF TOLERANCE — auto-flagged FAIL. Foreman alerted." Within-tolerance shows a green confirm.
  - `create_entry` re-evaluates authoritatively on submit: an out reading forces `status=fail` + `out_of_tolerance=true`, marks the entry failed, and posts an **alert** to the Live Feed tagging @foreman. (`ValidationCheck.out_of_tolerance` added.)
- **Live Feed** (`frontend/src/components/LiveFeed.jsx`, nav under Overview): company-wide chat (bubbles, avatars, @mention highlighting, **bold**), polls every 6s. Renders 4 message types: message / system / alert (red) / ai (blue with action chips). Backend: `GET/POST /api/feed`, `FeedMessage` collection.
- **@titanicf_bot BYOK assistant** (`backend/feed_ai.py`): mention the handle in the feed to trigger the AI. Uses `emergentintegrations` `LlmChat` with the USER's own key + chosen provider/model, `send_message_with_tools` tool-calling loop (per verified playbook). 7 tools give full CRUD: get_overview, list_tasks, create_job, add_task, add_validation_rule, recategorize_tasks, update_roi_settings (dispatched by `ai_dispatch` in server.py, resolves jobs/tasks by fuzzy name).
  - **AI Settings** panel (Admin → AI Settings): pick provider (OpenAI/Anthropic/Gemini) + model, set handle, paste API key (stored server-side, returned masked). `GET/POST /api/ai-settings`. Settings model gained `ai_provider`, `ai_api_key`, `bot_name`; default model `gpt-5.4`.
- **Tested**: tolerance parser unit-tested (8 cases correct); out-of-tolerance auto-fail + feed alert verified via curl (crew marked PASS, 3/8" auto-flipped to FAIL + alert posted); feed post + bot-not-connected reply verified via curl; Live Feed + AI Settings + mobile field OOT warning verified via screenshots. **AI happy-path (bot executing tool calls) NOT exercised because the Emergent universal key's budget is exhausted account-wide — this is expected since the user chose BYOK; the loop follows the verified playbook and error handling posts a clean message to the feed.**
