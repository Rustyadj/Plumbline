...
- **LLM**: User-selected OpenAI, Anthropic, or Gemini model through LiteLLM (BYOK).
...
- Provider-neutral AI calls supported through the user's configured API key.
...
- **@titanicf_bot BYOK assistant** (`backend/feed_ai.py`): mention the handle in the feed to trigger the AI. Uses LiteLLM with the user's own key and chosen provider/model. Seven tools provide CRUD: get_overview, list_tasks, create_job, add_task, add_validation_rule, recategorize_tasks, update_roi_settings (dispatched by `ai_dispatch` in server.py, resolves jobs/tasks by fuzzy name).
