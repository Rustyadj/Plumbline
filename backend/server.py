...
from feed_ai import complete_text, run_agent as run_ai_agent
...
async def llm_chat(system: str, user: str, session_id: str) -> str:
    ...
    if not settings.ai_api_key:
        raise HTTPException(503, "Add an AI API key in Admin → AI Settings first")
...
async def seed_demo(force: bool = False):
    ...
    job = Job(
        ...
        status="active",
        budget_hours=3200.0,
        cover_image="/assets/plumbline-jobsite.jpeg",
    )
    await db.jobs.insert_one(job.model_dump())
